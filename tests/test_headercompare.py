...
from retrievalist_parsers.hierarchy.headercompare import (
    condition_h1_enum_h2_not,
    condition_h2_extends_h1,
)
from retrievalist_parsers.model.document import Section, TextElement
from retrievalist_parsers.model.style import Style, TextSize


class TestSubHeaderConditions(TestCase):
    style_middle_bold = Style(
        bold=True,
        italic=True,
        font_name="test-font",
    ...
    def create_char(self, text):
        return LTChar(
            (1, 2, 3, 4, 5, 6),
            TestSubHeaderConditions.TestFont(),
            10,
            10,
            10,
            text,
    ...
    def test_condition_h2_extends_h1(self):
        element1 = TextElement(
            text_container=self.create_container("1.1 This is a test header"),
            style=self.style_middle_bold,
        )

        element2 = TextElement(
            text_container=self.create_container("1.1.2 This is a subheader of 1.1"),
            style=self.style_middle_bold,
        )
    ...
    def test_condition_enumeration__elements_with_same_style(self):
        ...
        subheader = TextElement(
            text_container=self.create_container("This is a subheader of 1.1"),
            style=self.style_middle_bold,
        )

        neighbor_element = TextElement(
            text_container=self.create_container("1.2 this is NOT a subheader of 1.1"),
            style=self.style_middle_bold,
        )

        self.assertTrue(condition_h1_enum_h2_not(Section(h1), Section(subheader)))
        self.assertFalse(
            condition_h1_enum_h2_not(Section(h1), Section(neighbor_element))
        )
